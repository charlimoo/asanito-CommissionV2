...
# --- THE MAIN TEST FUNCTION (SIMPLIFIED AND CORRECTED) ---
...
def test_calculation_engine_with_demo_data(demo_dataframes, app_with_db):
    ...
    # (This is the replacement for mocking)
    settings_data = {
        'CURRENCY_CONVERSION_FACTOR': ['0.1', 'float'], 'RENEWAL_COMMISSION_RATE': ['0.05', 'float'],
        'BRACKET_QUALIFICATION_MIN_COLLECTION_PERCENT': ['0.30', 'float'], 'AGENT_SALE_MULTIPLIER': ['0.5', 'float'],
        'DEFAULT_COMMISSION_MODEL': ['پورسانت خالص', 'string'],
        'AGENT_KEYWORDS': [json.dumps(['نمایندگان', 'نماینده'], ensure_ascii=False), 'json'],
    ...
    TOLERANCE = 0.01
    amanj_expected = {'original_commission': 14_400_000, 'bonus': 12_000_000, 'total_payable': 26_400_000, 'paid': 1_000_000, 'remaining': 25_400_000}
    parinaz_expected = {'original_commission': 500_000, 'bonus': 0, 'total_payable': 500_000, 'paid': 0, 'remaining': 500_000}
